        ...
        for (const auto& robot : robots) {
            ...
            if (dir == 'R') {
                ...
                while (!st.empty() && get<2>(st.top()) == 'R') {
                    ...
                    int topPos = get<0>(topRobot);
                    int topHealth = get<1>(topRobot);
                    int topIdx = get<3>(topRobot);
                    ...
                    if (topHealth < health) {
                        ...
                        st.pop();
                        st.push(make_tuple(topPos, topHealth - 1, 'R', topIdx));
                        health = 0;
                        break;
                    }
                }
                ...
                if (health > 0) {
                    st.push(make_tuple(pos, health, 'L', idx));
                }
            }
        }
       
        vector<int> tempHealths(n, -1);
        while (!st.empty()) {
            auto robot = st.top();
            st.pop();
            int pos = get<0>(robot);
            ...
            tempHealths[idx] = health;
        }
        
        for (int i = 0; i < n; ++i) {
            if (tempHealths[i] != -1) {
                result.push_back(tempHealths[i]);
            }
        }
